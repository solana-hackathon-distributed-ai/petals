...
app.post('/sol', async (req, res) => {
    try {
        ...
        const recipientTokenAccount = await getAssociatedTokenAddress(mintAddress, recipientPublicKey, false // allowOwnerOffCurve
        );
        ...
        for (let i = 0; i < Number(num_blocks); i++) {
            ...
            if (!recipientAccountExists && i === 0) {
                transaction.add(createAssociatedTokenAccountInstruction(keypair.publicKey, // payer
            ...
            transaction.add(createTransferInstruction(sourceTokenAccountAddress, recipientTokenAccount, // Using associated token account
            keypair.publicKey, BigInt(amount_per_block) * BigInt(1e9)));
            try {
...
app.post('/chat', async (req, res) => {
    try {
        ...
        const sourceTokenAccountAddress = await getAssociatedTokenAddress(mintAddress, new PublicKey(source), false // allowOwnerOffCurve if necessary
        );
        try {
        ...
        // Add transfer instruction
        transaction.add(createTransferInstruction(sourceTokenAccountAddress, recipientTokenAccount, // Using the recipient's associated token account
        keypair.publicKey, totalAmount.toString()));
        try {
            const signature = await connection.sendTransaction(transaction, [keypair], {
                skipPreflight: false,
                preflightCommitment: 'confirmed'
